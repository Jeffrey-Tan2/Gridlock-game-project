...
public class LevelComponent implements Component {
	...
	private final ArrayList<Movement> playbackHistory;
	...
	public LevelComponent(Level levelSnapshot, ArrayList<Movement> playbackHistory, Runnable victoryHandler) {
	...
	public void init() {
		...
		bgLayer = new Pane();
		Image dirtbg = new Image("/gridlock/ui/sprites/dirtbg.png");
	...
	private void playbackLoop() {
		while (!playbackHistory.isEmpty() && !Thread.interrupted()) {
			try {
				Thread.sleep(500);
				Movement movement = playbackHistory.remove(0);
				VehicleComponent vc = vehicleComponents[movement.getVehicle().getId() - 1];
				vc.redo(movement);
			} catch (InterruptedException e) {
				break;
			}
		}
		TranslateTransition transition = new TranslateTransition(Duration.millis(1000), vehicleComponents[0].getNode());
		transition.setByX(Constants.WINDOW_WIDTH);
		transition.play();
		transition.setOnFinished(event -> {
			victoryHandler.run();
		});
